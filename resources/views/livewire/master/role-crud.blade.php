            ...
            @auth
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-white mb-1">
                        Manajemen Role 🎭 — {{ auth()->user()->username }}
                    </h1>
            ...
            <section class="bg-white rounded-2xl shadow-xl overflow-hidden border-t-4 border-indigo-600">
                <div class="bg-gradient-to-r from-indigo-50 to-blue-50 px-6 py-5 border-b border-gray-200 flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <h2 class="text-lg font-bold text-gray-800">Daftar Role</h2>
                            <p class="text-xs text-gray-600">Total: <span class="font-semibold">{{ count($data) }}</span> role</p>
                        </div>
                    </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="bg-gradient-to-r from-gray-50 to-gray-100">
                                <th class="px-4 py-3 border-b-2 border-gray-200 text-left font-bold text-xs text-gray-700 uppercase">ID Role</th>
                                <th class="px-4 py-3 border-b-2 border-gray-200 text-left font-bold text-xs text-gray-700 uppercase">Nama Role</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse($data as $r)
                                <tr class="hover:bg-blue-50 transition">
                                    ...
                                    <td class="px-4 py-3 font-medium text-gray-800">{{ $r->nama_role }}</td>
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex justify-center gap-2">
                                            <button type="button" wire:click="edit({{ $r->idrole }})"
                                                @click="$wire.edit({{ $r->idrole }}).then(() => openModal())"
                                                class="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold rounded-lg shadow transition">
                                                ✏️ Edit
                                            </button>
                                            <button onclick="confirmDelete({{ $r->idrole }}, '{{ $r->nama_role }}')"
                                ...
                                <tr>
                                    <td colspan="3" class="px-4 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center text-gray-500">
                                            ...
                                            <p class="font-medium text-sm">Belum ada data role</p>
                                            <p class="text-xs text-gray-400 mt-1">Klik tombol "Tambah Role" untuk menambahkan data</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
        ...
        <div id="modalContent"
            ...
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-5 border-b border-gray-200 rounded-t-2xl flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">{{ $isEdit ? 'Edit Role' : 'Tambah Role' }}</h3>
                        <p class="text-xs text-gray-500">{{ $isEdit ? 'Perbarui data role' : 'Tambah role baru' }}</p>
                    </div>
                </div>
            ...
            <form wire:submit.prevent="{{ $isEdit ? 'update' : 'store' }}" class="p-6 space-y-4">
                {{-- Nama Role --}}
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Nama Role <span class="text-rose-500">*</span></label>
                    <div class="relative">
                        ...
                        <input type="text" wire:model.defer="nama_role" placeholder="Contoh: Admin, User, Manager" id="focusNamaRole"
                            class="w-full pl-10 pr-3 py-2 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition">
                    </div>
                    @error('nama_role')
        ...
        <div id="deleteModalContent"
            ...
            <div class="p-6">
                ...
                <p class="text-gray-600 text-center mb-2">
                    Apakah Anda yakin ingin menghapus role <br>
...
<script>
    ...
    function openModal() {
        ...
        requestAnimationFrame(() => {
            ...
            setTimeout(() => {
                const firstInput = document.getElementById('focusNamaRole');
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.addEventListener('keydown', function(e) {
            if (e.key === 'Escape') {
                ...
                const deleteShown = deleteModalContainer && !deleteModalContainer.classList.contains('hidden');

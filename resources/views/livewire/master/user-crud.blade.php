            ...
            @auth
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-white mb-1">
                        Manajemen User 👥 — {{ auth()->user()->username }}
                    </h1>
                    <p class="text-blue-100 text-sm">Kelola data pengguna dan akses sistem</p>
                ...
                <div class="bg-gradient-to-r from-indigo-50 to-blue-50 px-6 py-5 border-b border-gray-200 flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-600">Total: <span class="font-semibold">{{ count($data) }}</span> pengguna</p>
                        </div>
                    </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="bg-gradient-to-r from-gray-50 to-gray-100">
                                ...
                                <th class="px-4 py-3 border-b-2 border-gray-200 text-left font-bold text-xs text-gray-700 uppercase">Username</th>
                                <th class="px-4 py-3 border-b-2 border-gray-200 text-left font-bold text-xs text-gray-700 uppercase">Role</th>
                                ...
                                <tr class="hover:bg-blue-50 transition">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex justify-center gap-2">
                                            <button type="button" wire:click="edit({{ $u->iduser }})"
                                                @click="$wire.edit({{ $u->iduser }}).then(() => openModal())"
                                                class="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold rounded-lg shadow transition">
                                                ✏️ Edit
                                            </button>
                                            <button onclick="confirmDelete({{ $u->iduser }}, '{{ $u->username }}')"
                                ...
                                <tr>
                                    <td colspan="4" class="px-4 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center text-gray-500">
                                            ...
                                            <p class="font-medium text-sm">Belum ada data user</p>
                                            <p class="text-xs text-gray-400 mt-1">Klik tombol "Tambah User" untuk menambahkan data</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
    ...
    <div id="modalContainer" wire:ignore.self class="fixed inset-0 flex justify-center items-center hidden z-50 p-4">
        <div id="modalContent"
            ...
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-5 border-b border-gray-200 rounded-t-2xl flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500">{{ $isEdit ? 'Perbarui data pengguna' : 'Tambah pengguna baru' }}</p>
                    </div>
                </div>
            ...
            <form wire:submit.prevent="{{ $isEdit ? 'update' : 'store' }}" class="p-6 space-y-4">
                {{-- Username --}}
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Username <span class="text-rose-500">*</span></label>
                    <input type="text" wire:model.defer="username" placeholder="Masukkan username" id="focusUsername"
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">
                        Password 
                        @if(!$isEdit)<span class="text-rose-500">*</span>@endif
                    </label>
                    <input type="password" wire:model.defer="password" 
                        placeholder="{{ $isEdit ? 'Kosongkan jika tidak diganti' : 'Masukkan password' }}"
                        class="w-full px-3 py-2 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition">
                    @error('password')
                        <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                    @enderror
                ...
                {{-- Role --}}
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Role <span class="text-rose-500">*</span></label>
                    <select wire:model.defer="idrole"
                        class="w-full px-3 py-2 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition">
...
<script>
    ...
    function openModal() {
        ...
        requestAnimationFrame(() => {
            ...
            setTimeout(() => {
                const firstInput = document.getElementById('focusUsername');
